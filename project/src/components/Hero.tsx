...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="min-h-screen relative flex items-center pt-20 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-navy-dark via-navy to-navy-light opacity-80 z-0"></div>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(64,158,255,0.1)_0%,rgba(0,0,0,0)_60%)] z-0"></div>
      
      <div className="container mx-auto px-6 py-12 z-10 relative">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="lg:pr-8">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
              <span className="text-white">Behind the Screens:</span>
              <br />
              <span className="text-teal">The Art</span> <span className="text-gold">of Digital Order</span>
            </h1>
          ...
          <div ref={panelsRef} className="relative h-[400px] md:h-[500px] w-full flex items-center justify-center">
            {/* Interactive RGB Layered Panels */}
            <div className="interactive-panel absolute w-[70%] h-[70%] bg-accent-blue/20 rounded-xl border border-accent-blue/30 shadow-xl backdrop-blur-sm transition-transform duration-300 ease-out"></div>
            <div className="interactive-panel absolute w-[80%] h-[80%] bg-accent-green/20 rounded-xl border border-accent-green/30 shadow-xl backdrop-blur-sm transition-transform duration-300 ease-out"></div>
            <div className="interactive-panel absolute w-[90%] h-[90%] bg-accent-red/20 rounded-xl border border-accent-red/30 shadow-xl backdrop-blur-sm transition-transform duration-300 ease-out"></div>
            ...
            <div className="absolute inset-10 grid grid-cols-3 grid-rows-3 gap-3 opacity-90">
              {Array.from({ length: 9 }).map((_, i) => (
                <div 
                  key={i}
                  className={`rounded-md border ${i % 3 === 0 ? 'border-accent-red/50 bg-accent-red/10' : i % 3 === 1 ? 'border-accent-green/50 bg-accent-green/10' : 'border-accent-blue/50 bg-accent-blue/10'} animate-pulse-slow`}
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white/70 animate-fade-down">
        <ChevronDown size={28} />
      </div>
    </section>
  );
